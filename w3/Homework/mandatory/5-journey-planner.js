...
*/
function getTransportModes(transportSelection) { 
  return transportSelection.slice(1)
}
...
*/
function isAccessibleByTransportMode(location, transportMode) { 
  return location.includes(transportMode)
}

console.log(isAccessibleByTransportMode(["tube", "bus"]));
...
function journeyPlanner(locations, transportMode) {
  let checkTransportMode = locations.filter(transport => transport.includes(transportMode));
  let locationSpeciTransport = checkTransportMode.map(location => location[0])
  return locationSpeciTransport  // Implement the function body
}
