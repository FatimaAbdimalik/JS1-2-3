/*
Following are set of small functions that you need to create using array methods 
...
function checkAllEven(arr) {
  ...
  //they can use filter and check length equal to orignal
}
...
function addIndexes(arr) {
  // let newArr = arr.map((p, i) => p + i);
  //this is not acceptable
  // let newArr = arr.map((p) => p + arr.indexOf(p)); shoud understand index of work

  var arrayAdded = [];
  for (let i = 0; i < arr.length; i++) {
    arrayAdded.push(arr[i] + i);
  }
